...
import {
  ...
  type GetOrganizerEventsResultQuery,
  type GetOrganizerEventsErrorQuery,
} from './get-organizer-events.query';

// Re-export types for external use
export type GetOrganizerEventsResult = GetOrganizerEventsResultQuery;
export type GetOrganizerEventsError = GetOrganizerEventsErrorQuery;
...
/**
 * Handler for GetOrganizerEventsQuery
 *
 * Follows CQRS pattern - retrieves paginated events for an organizer.
 *
 * Responsibilities:
 * 1. Validate organizer ID
 ...
 */
@Injectable()
export class GetOrganizerEventsHandler {
  private readonly logger = new Logger(GetOrganizerEventsHandler.name);
  ...
  /**
   * Execute the get organizer events query
   */
  async execute(
    query: GetOrganizerEventsQuery,
  ): Promise<Result<GetOrganizerEventsResult, GetOrganizerEventsError>> {
    this.logger.debug(`Getting events for organizer: ${query.organizerId}`);

    // ============================================
    // 1. Validate organizer ID
    // ============================================
    if (!query.organizerId || query.organizerId.trim() === '') {
      return Result.fail({
        type: 'INVALID_ORGANIZER_ID',
    ...
    // 3. Query repository
    // ============================================
    const result = await this.eventRepository.findByOrganizerId(
      query.organizerId,
      options,
    );
