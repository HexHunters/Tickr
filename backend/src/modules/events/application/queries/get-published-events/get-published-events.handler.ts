...
import {
  ...
  type GetPublishedEventsResultQuery,
} from './get-published-events.query';

// Re-export types for external use
export type GetPublishedEventsResult = GetPublishedEventsResultQuery;
...
/**
 * Handler for GetPublishedEventsQuery
 *
 * Follows CQRS pattern - retrieves paginated published events with filters.
 *
 * Responsibilities:
...
export class GetPublishedEventsHandler {
  ...
  /**
   * Execute the get published events query
   */
  async execute(
    query: GetPublishedEventsQuery,
  ): Promise<Result<GetPublishedEventsResult, never>> {
    this.logger.debug('Getting published events with filters', query.filters);
    ...
    // ============================================
    // 3. Query repository (findPublished only returns PUBLISHED events)
    // ============================================
    const result = await this.eventRepository.findPublished(filters, options);
    ...
    this.logger.debug(`Found ${result.total} published events`);
